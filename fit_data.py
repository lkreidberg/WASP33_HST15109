...
def residuals(params, data, flags, model, myfuncs, fjac=None):			
    print "FIXME: what is model?"
    print model
    fit = model.fit(params, data, myfuncs)
    print "FXIME: what is fit?"
    print fit
    return [0, fit.resid/data.err]

def least_sq_fit(file_name, obs_par, fit_par, data, flags, model, myfuncs):
    ...
    model = Model(params_s, data ,flags, myfuncs)
    
    if flags['plot-raw-data']: plot_raw(data)
    fa = {'data':data, 'flags':flags, 'model':model, 'myfuncs':myfuncs}
...
#		print "subtracting 2 from dof for divide-white"
    ...
    if flags['output']: 
            ...
            #print>>f, "{0:0.3f}".format(data.wavelength), "{0:0.6f}".format(m.params[data.par_order['rp']*nvisit]), "{0:0.6f}".format(m.perror[data.par_order['rp']*nvisit]), "{0:0.2f}".format(bestfit.chi2red), "{0:0.3f}".format(m.params[data.par_order['u1']*nvisit]), "{0:0.3f}".format(m.perror[data.par_order['u1']*nvisit])
            print>>f, "{0:0.3f}".format(data.wavelength), "{0:0.6f}".format(m.params[data.par_order['rp']*nvisit]), "{0:0.6f}".format(m.perror[data.par_order['rp']*nvisit]), "{0:0.3f}".format(m.params[data.par_order['u1']*nvisit]), "{0:0.3f}".format(m.params[data.par_order['u2']*nvisit]), "{0:0.2f}".format(bestfit.chi2red)
            pickle.dump([data, bestfit], open("white_lc_fit.p", "wb"))
            f.close()


    if flags['verbose']: 
            print "{0:0.3f}".format(data.wavelength), "{0:0.2f}".format(bestfit.chi2red)#, m.params[data.par_order['amp1']*nvisit], m.perror[data.par_order['amp1']*nvisit]
            PrintParams(m, data)

    if flags['show-plot']: plot_fit(m.params, data, flags, obs_par, myfuncs)
...
def main():
    ...
    verbose, output, show_plot, run_mcmc, run_lsq, plot_raw_data, plot_sys, \
        path, fit_white, divide_white = \
        False, False, False, False, True, False, False, "spec_lc", False, False
    ...
    flags = {'verbose': verbose, 'show-plot': show_plot, 
            'plot-raw-data': plot_raw_data, 'plot-sys': plot_sys, 
            'output': output, 'out-name': 'none.txt', 'run-lsq': run_lsq, 
            'run-mcmc': run_mcmc, 'divide-white': divide_white, 
            'fit-white': fit_white}		

    #reads in observation and fit parameters
    obs_par = make_dict(ascii.read("config/obs_par.txt", 
                        Reader=ascii.CommentedHeader))
    fit_par = ascii.read("config/fit_par.txt", 
                        Reader=ascii.CommentedHeader)
    ...
    for f in files:
        ...
        if flags['divide-white']:
            ...
            data.all_sys = sys_vector
        params = []
        model = Model(params, data, flags, myfuncs)
        data, params = least_sq_fit(f, obs_par, fit_par, data, flags, model, myfuncs)
