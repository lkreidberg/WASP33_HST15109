...
for i in enumerate(files):
	...
	if(ima[0].header['filter'] == obs_par['FILTER']):
		...
		ima[1].data =  ima[1].data*ima[0].header['exptime']		#multiply by exptime to convert data to counts 
		...
		if diagnostics==True: 
			...
			plt.imshow(dat*ima[0].header['exptime'], origin ='lower',vmin=0, vmax=1000)
		...
		if output==True: 
			print>>f, t, results[3]+rmin-LTV1, results[2]+cmin-LTV2  
			if t - t_last > 5./24.:							#checks if this image corresponds to a new visit
				print>>tstart, np.floor(t)	
				t_last = t
