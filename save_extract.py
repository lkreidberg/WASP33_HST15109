...
def plot_spectrum(spec_opt, ancil):
	...
	"""m = np.load("W103_phoenix_model.npy")
	w = m[0]*1.0e6
	idx = (w > 1.0)&(w < 2.0)
	w = w[idx]
	model = m[1][idx]
...
def plot_trace(d, ancil):
	...
	orbnum = ancil.orbnum
	trace_i = ancil.refpix[orbnum,2] + float(ancil.BEAMA_i + ancil.LTV1)		#start of trace
	trace_f = ancil.refpix[orbnum,2] + float(ancil.BEAMA_f + ancil.LTV1)		#end of trace
...
def get_wave_grid(ancil):
	wave_grid = np.empty((ancil.norb, ancil.subarray_size, ancil.subarray_size))
	...
	for i in range(ancil.norb):
		for j in range(ancil.subarray_size):
			...
			delx = 0.5 + np.arange(ancil.subarray_size) - (ancil.refpix[i,2] + ancil.LTV1) 
...
for f in ancil.files:
	...
	if ancil.plot_trace: plot_trace(d, ancil)

	cmin = int(ancil.refpix[ancil.orbnum,2] + ancil.POSTARG1/ancil.platescale) + ancil.BEAMA_i + ancil.LTV1 				#determines left column for extraction (beginning of the trace)
	cmax = min(int(ancil.refpix[ancil.orbnum,2] + ancil.POSTARG1/ancil.platescale) + ancil.BEAMA_f + ancil.LTV1, ancil.subarray_size) #right column (end of trace, or edge of detector)
	...
	print M.shape, rmin, rmax, cmin, cmax, ancil.LTV1, ancil.LTV2

	bpix = d[3].data[rmin:rmax,cmin:cmax]
	badpixind =  (bpix==4)|(bpix==512)|(flatfield[ancil.orbnum][rmin:rmax, cmin:cmax] == -1.)	#selects bad pixels; FIXME: this is not right!!
	...
	print "do you want to correct for wavelength shfit?"
	shift = 0.
	...
	print nspectra, time[0], sum(spec_opt), ancil.visnum, f, shift
